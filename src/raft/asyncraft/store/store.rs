...
use super::{innerstore::{InnerStore, StoreRequest, StoreResponse}, ShutdownError};
...
impl RaftStorage<ClientRequest, ClientResponse> for AStore{
    ...
    async fn get_membership_config(&self) -> anyhow::Result<MembershipConfig> {
        match self.send_store_msg(StoreRequest::GetMembershipConfig).await? {
            StoreResponse::MembershipConfig(v) => {
                Ok(v)
            }
    ...
    async fn save_hard_state(&self, hs: &HardState) -> anyhow::Result<()> {
        match self.send_store_msg(StoreRequest::SaveHardState(hs.to_owned())).await? {
            StoreResponse::None => {
                Ok(())
            }
            _ => Err(self.store_response_err())
    ...
    async fn get_log_entries(&self, start: u64, stop: u64) -> anyhow::Result<Vec<Entry<ClientRequest>>> {
        match self.send_store_msg(StoreRequest::GetLogEntries {start,stop}).await? {
            StoreResponse::LogEntries(v) => {
                Ok(v)
            }
            _ => Err(self.store_response_err())
        }
    ...
    async fn delete_logs_from(&self, start: u64, stop: Option<u64>) -> anyhow::Result<()> {
        match self.send_store_msg(StoreRequest::DeleteLogsFrom{start,stop}).await? {
            StoreResponse::None => {
                Ok(())
            }
            _ => Err(self.store_response_err())
    ...
    async fn append_entry_to_log(&self, entry: &Entry<ClientRequest>) -> anyhow::Result<()> {
        match self.send_store_msg(StoreRequest::AppendEntryToLog(entry.to_owned())).await? {
            StoreResponse::None => {
                Ok(())
            }
            _ => Err(self.store_response_err())
    ...
    async fn replicate_to_log(&self, entries: &[Entry<ClientRequest>]) -> anyhow::Result<()> {
        ...
        match self.send_store_msg(StoreRequest::ReplicateToLog(entries)).await? {
            StoreResponse::None => {
                Ok(())
            }
            _ => Err(self.store_response_err())
    ...
    async fn replicate_to_state_machine(&self, entries: &[(&u64, &ClientRequest)]) -> anyhow::Result<()> {
    ...
    async fn finalize_snapshot_installation(&self, index: u64, term: u64, delete_through: Option<u64>, id: String, snapshot: Box<Self::Snapshot>) -> anyhow::Result<()> {
        match self.send_store_msg(StoreRequest::FinalizeSnapshotInstallation {index,term,delete_through,id,snapshot}).await? {
            StoreResponse::None => {
                Ok(())
            }
            _ => Err(self.store_response_err())
    ...
    async fn get_current_snapshot(&self) -> anyhow::Result<Option<CurrentSnapshotData<Self::Snapshot>>> {
        match self.send_store_msg(StoreRequest::GetCurrentSnapshot).await? {
            StoreResponse::OptionSnapshot(v) => {
                Ok(v)
            }
            _ => Err(self.store_response_err())
        }
