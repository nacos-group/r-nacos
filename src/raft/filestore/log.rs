    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(16) => msg.start_index = r.read_uint64(bytes)?,
                Ok(24) => msg.record_count = r.read_uint64(bytes)?,
                Ok(32) => msg.is_close = r.read_bool(bytes)?,
                Ok(40) => msg.mark_remove = r.read_bool(bytes)?,
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl MessageWrite for LogRange {
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.start_index != 0u64 { w.write_with_tag(16, |w| w.write_uint64(*&self.start_index))?; }
        if self.record_count != 0u64 { w.write_with_tag(24, |w| w.write_uint64(*&self.record_count))?; }
        if self.is_close != false { w.write_with_tag(32, |w| w.write_bool(*&self.is_close))?; }
        if self.mark_remove != false { w.write_with_tag(40, |w| w.write_bool(*&self.mark_remove))?; }
        Ok(())
    }
}
...
impl<'a> MessageRead<'a> for RaftIndex {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Self::default();
        while !r.is_eof() {
            match r.next_tag(bytes) {
...
impl MessageWrite for RaftIndex {
    fn get_size(&self) -> usize {
        0
