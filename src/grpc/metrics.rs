    ...
    fn handle(&mut self, _msg: MetricsQuery, _ctx: &mut Self::Context) -> Self::Result {
        let list = vec![
            MetricsItem {
                ...
                record: MetricsRecord::Gauge(self.conn_cache.len() as f64),
            },
            MetricsItem {
                metrics_type: MetricsKey::GrpcConnActiveTimeoutSetItemSize,
                record: MetricsRecord::Gauge(self.active_time_set.item_size() as f64),
            },
            MetricsItem {
                metrics_type: MetricsKey::GrpcConnResponseTimeoutSetItemSize,
                record: MetricsRecord::Gauge(self.response_time_set.item_size() as f64),
            },
        ];
        Ok(list)
    }
