use goose::{ prelude::* };
use rand::{prelude::{StdRng, Distribution}, distributions::Uniform, SeedableRng};
use std::{
    sync::{Arc, Mutex},
    time::{Duration},
};
...
impl NamingUser {
    pub fn new(id:u64,port:u16,service_names:Arc<Vec<Arc<String>>>) -> Self{
        ...
        for item in service_names.as_ref() {
            let beat_data = BEAT_DATA_FMT.replace("{service_name}",&item).replace("{port}", &port_str);
