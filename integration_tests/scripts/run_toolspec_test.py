#!/usr/bin/env python3
"""
ToolSpec Console API Test Runner

This script starts a single r-nacos instance and runs the ToolSpec console API tests.
It's designed to be simple and focused on testing the ToolSpec API functionality.
"""
...
def wait_for_server(host: str = "127.0.0.1", port: int = 8848, timeout: int = 30):
    ...
    url = f"http://{host}:{port}/rnacos/api/console/v2/toolspec/list"
...
def run_toolspec_tests(host: str = "127.0.0.1", port: int = 8848) -> int:
    """Run the ToolSpec API tests"""
    test_script = SCRIPT_DIR / "toolspec_console_api_test.py"
    
    if not test_script.exists():
        print(f"❌ Test script not found: {test_script}")
        return 1
    
    print("🧪 Running ToolSpec Console API tests...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ToolSpec Console API Test Runner")
